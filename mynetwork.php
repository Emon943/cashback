  <?php include 'user/sub-header.php';?>

            <ul id="logged_nav">
                <li id="shop_nav"><a href="retailers.php"></a></li>
                <li id="coupons_nav"><a href="coupons.php"></a></li>
                <li id="money_nav"><a href="getpaid.php"></a></li>
                <li id="invite_nav"><a href="invite.php"></a></li>
                            </ul>

            <div class="clear"></div>
        </div>
    </div>

    <div id="available_balance_bar">
        <div class="wrap">
            <div class="innerwrap">
                <div class="left">
                    <h1>Available Balance: <span style='cursor: pointer;' class='ttip'> <img src="images/icons/tt_btn.png" /> <span>Available Balace is the amount of funds you may  withdraw.  Available Balance does not include Pending  Cash Back, Cashout Requested, or Year to Date paid funds. <b></b> </span> </span></h1>

                    <div class="funds_info">
                        <div class="fund_amount">
                            <span>$0.00</span>
                        </div>
                        <a href="withdraw.php">Withdraw Funds</a>
                    </div>

                    <div class="clear"></div>
                </div>
                <div class="right">
                    <span class="money_status"><strong>Pending Cash Back:</strong> <span style='cursor: pointer;' class='ttip'> <img src="images/icons/tt_btn.png" /> <span>
                        <p>"Pending" Cash Back are transactions that merchants have reported to us.  It remains as “Pending” to allow time for returns, exchanges, or cancelled orders.</p><br /> 
                        <p>When the transaction fully clears, the “Pending” cash back is then posted to your “Available” cash back.  You are able to then request payment.</p> <br /> 
                        <p>Travel reservations move from "Pending" to your "Available Balance" upon completion of travel itinery.</p> <b></b> </span> </span> 
                    </span><span class="greens">$0.00</span><br />


                    <span class="money_status">Cashout Requested:</span><span style='cursor: pointer;' class='ttip'> <img src="images/icons/tt_btn.png" /> <span><p>You may request payment in any amount of $50 or more.</p><b></b> </span> </span> 
                     <span class="greens">$0.00</span><br />
                    <span class="money_status">Lifetime Cash Back: <span style='cursor: pointer;' class='ttip'> <img src="images/icons/tt_btn.png" /> <span><p>Lifetime Cash Back is the total amount of your "Available Balance" and the total amount of all paid "Withdrawls".</p><b></b> </span> </span> 
                        <span class="greens">$0.00</span><br />
                </div>

                <div class="clear"></div>
            </div><!-- .innerwrap -->

            <div class="clear"></div>
        </div>
    </div>

    <div class="clear"></div>

</div><!-- #header -->

<!-- Sidebar/Left Column/Wrap Start for Layout -->
<div class="wrap">
    <div id="column_left">
       <div class="box">
            <div class="top">
                Shop by Category

                <a href="#" class="expand_contract_mobile"></a>
            </div>
             <?php include 'category.php';?>
            <div class="bottom">&nbsp;</div>
        </div>
        
        <div class="banner_left_ads">
             
            <a href="http://www.cashbackkaboom.com/getpaid.php" ><img src="img/uploads/image-link//img/uploads/image-link//img/uploads/image-li" style="max-width:100%;" /></a>
        </div>
    </div>

<div id="column_center">
<script type="text/javascript">
function showTable1(){
    document.getElementById('referred_shoppers').style.display = "block";
    document.getElementById('shopping_levels').style.display = "none";
    document.getElementById('shopper_refer_btn').style.background = "#D5D3C6";
    document.getElementById('shopper_lvl_btn').style.background = "#F9F9F7";
    document.getElementById('shopper_lvl_btn').style.fontWeight = "normal";
    document.getElementById('shopper_refer_btn').style.fontWeight = "bold";
}

function showTable2(){
    document.getElementById('referred_shoppers').style.display = "none";
    document.getElementById('shopping_levels').style.display = "block";
    document.getElementById('shopper_refer_btn').style.background = "#F9F9F7";
    document.getElementById('shopper_lvl_btn').style.background = "#D5D3C6";
    document.getElementById('shopper_lvl_btn').style.fontWeight = "bold";
    document.getElementById('shopper_refer_btn').style.fontWeight = "normal";
    document.getElementById('shopper_lvl_btn').style.fontWeight = "bold";
}
</script>
<div id="network_page">
    <div class="network_header">
        <h1 class="pagetitle" style="margin-bottom:30px;">Let's Make Some Money!</h1>

        <p><img src="images/small_stack.png" alt="Let's Make Some Money!" class="right small_stack" />Congratulations on spreading the good news about the great Money Saving - Money Making benefits Cash Back Kaboom offers!</p>

        <p>Here you'll find a list of all the shoppers you have referred.  Plus, click All My Shopping Levels to view the total shoppers and cash back in your Shopping Network.</p>

        <p>Keep on sharing and inviting - and watch your shopping network and cash back go Kaboooooom!</p>
    </div>

        <div class="table_head_border">
            <h3 style="float:left;" class="section">My Shopping Network</h3>

            <div id="network_choices">
                <span id="shopper_refer_btn" onclick="showTable1()" style="position: relative; left: 3px;font-weight: bold;">My Referred Shoppers</span> <span id="shopper_lvl_btn" onclick="showTable2()">All My Shopping Levels</span>
            </div>

            <div style="clear:both;"></div>
        </div><!-- .table_head_border -->

    <a name="referrals"></a>
            <div id="referred_shoppers">
                <table align="center" width="100%" border="0" cellspacing="0" cellpadding="5" class="my_account_table">
                <tr>
                    <th class="network_col_header" width="27%">&nbsp;&nbsp;My Username</th>
                    <th class="network_col_header" width="27%">Signup Date</th>
                    <th class="network_col_header" width="23%">Total Referrals</th>
                    <th class="network_col_header" width="23%">Pending Earnings</th>
                    <th class="network_col_header" width="23%">Confirmed Earnings</th>
                </tr>
                <tr id="network_table_1">
                    <td>&nbsp;&nbsp;Motiur Rahman</td>
                    <td>2017-01-11 01:55:35</td>
                    <td>&nbsp;0</td>
                    <td>&nbsp;$0.00</td>
                    <td>&nbsp;$0.00</td>
                </tr>
                </table>

                <br />

                                    <p>You have no shopping tree at this time.</p>
                            </div>
            
            <div id="shopping_levels" style="display:none;">
            
                <table align="center" width="100%" border="0" style="border-bottom: solid 1px #EEEEEE;" cellspacing="0" cellpadding="3" class="my_account_table">
                  <tr>
                    <th class="network_col_header" width="25%">My Shopping Levels</th>
                    <th class="network_col_header" width="25%">Shoppers per Level</th>
                    <th class="network_col_header" width="25%">Cash Back Amount <span style='cursor: pointer;' class='ttip'> <img src="images/icons/tt_btn.png" /> <span>
                        <p>"Pending" Cash Back are transactions that merchants have reported to us.  It remains as “Pending” to allow time for returns, exchanges, or cancelled orders.</p><br /> 
                        <p>When the transaction fully clears, the “Pending” cash back is then posted to your “Available” cash back.  You are able to then request payment.</p> <br /> 
                        <p>Travel reservations move from "Pending" to your "Available Balance" upon completion of travel itinery.</p> <b></b> </span> </span> 
</th>
                    <th class="network_col_header" width="25%">Earned Cash Back</th>
                  </tr>
                  <tr class="row_even">
                    <td style="color: #097D2C; background: #FAFAFA;">Level 1</td>
                    <td style="color: #848482; background: #FAFAFA;">You</td> 
                    <td style="color: #848482; background: #FAFAFA;">$0.00</td>
                    <td style="color: #848482; background: #FAFAFA;">$0.00</td>
                  </tr>
                                  <tr class="row_even">
                    <td style="color: #097D2C;">Level 2</td>
                    <td style="color: #848482;">0</td>
                    <td style="color: #848482;">$0.00</td>
                    <td style="color: #848482;">$0.00</td>
                  </tr>
                                  <tr class="row_odd">
                    <td style="color: #097D2C;">Level 3</td>
                    <td style="color: #848482;">0</td>
                    <td style="color: #848482;">$0.00</td>
                    <td style="color: #848482;">$0.00</td>
                  </tr>
                                  <tr class="row_even">
                    <td style="color: #097D2C;">Level 4</td>
                    <td style="color: #848482;">0</td>
                    <td style="color: #848482;">$0.00</td>
                    <td style="color: #848482;">$0.00</td>
                  </tr>
                                  <tr class="row_odd">
                    <td style="color: #097D2C;">Level 5</td>
                    <td style="color: #848482;">0</td>
                    <td style="color: #848482;">$0.00</td>
                    <td style="color: #848482;">$0.00</td>
                  </tr>
                                  <tr class="row_even">
                    <td style="color: #097D2C;">Level 6</td>
                    <td style="color: #848482;">0</td>
                    <td style="color: #848482;">$0.00</td>
                    <td style="color: #848482;">$0.00</td>
                  </tr>
                                  <tr class="row_odd">
                    <td style="color: #097D2C;">Level 7</td>
                    <td style="color: #848482;">0</td>
                    <td style="color: #848482;">$0.00</td>
                    <td style="color: #848482;">$0.00</td>
                  </tr>
                                  <tr class="row_even">
                    <td style="color: #097D2C;">Level 8</td>
                    <td style="color: #848482;">0</td>
                    <td style="color: #848482;">$0.00</td>
                    <td style="color: #848482;">$0.00</td>
                  </tr>
                                  <tr class="row_odd">
                    <td style="color: #097D2C;">Level 9</td>
                    <td style="color: #848482;">0</td>
                    <td style="color: #848482;">$0.00</td>
                    <td style="color: #848482;">$0.00</td>
                  </tr>
                                  <tr class="row_even">
                    <td style="color: #097D2C;">Level 10</td>
                    <td style="color: #848482;">0</td>
                    <td style="color: #848482;">$0.00</td>
                    <td style="color: #848482;">$0.00</td>
                  </tr>
                                  <tr>
                    <td style="color: #097D2C; background: #FAFAFA;" width="25%">Totals</td>
                    <td style="color: #848482; background: #FAFAFA;" width="25%">1</td>
                    <td style="color: #848482; background: #FAFAFA;" width="25%">$0.00</td>
                    <td style="color: #848482; background: #FAFAFA;" width="25%">$0.00</tD>
                  </tr>
                </table>
            </div>          
</div>
</div>


</div><!-- .wrap -->

<div id="need_more_cash">
	<a href="../invite.php"><img src="images/need_cash_back.png" alt="Need More Cash Back?" /></a><br />
	<a style="cursor:pointerl;" class="hide_link lo_link">Hide</a>
</div>

<div class="md-modal md-effect-8" id="modal-8">
	<div class="md-content">
		<div id="footer-logo">
			<img src="img/modal/logo.png" alt="logo" style="position:absolute;bottom:210px; left:-43px; width:27%"> 
		</div>

		<h3>Share with Friends!</h3>
		<img id="footer_divider" src="img/images/divider.jpg" alt="divider" />
		
		<div>
			<p class="blurb">Use Cash Back Kaboom’s fast and easy invitation methods to invite others. Those you invite will be added to your Shopping Network and you’ll get paid each time they shop - over and over again - forever!*</p>
			<div id="footer_form">
				<form name="footer_modal" action="#" method="post">
					<p><span class="footer_verb">Copy It! </span> <input id="copyit_txtbox" type="text" name="copy" readonly="readonly" onfocus="this.select();" onclick="this.focus();this.select();" value="http://www.cashbackkaboom.com/3735"></p><br />
					<p><span class="footer_verb">Email it!</span> <input id="fname_txtbox" type="text" name="friends_name" placeholder="Friend's Name"> &nbsp; <input id="femail_txtbox" type="text" name="friends_email" placeholder="Friend's Email"></p><br />
					<p><span class="footer_verb">Share It!</span> &nbsp; &nbsp; &nbsp; 
						<a href="http://cashbackkaboom.com/3735" onclick=" window.open('https://www.facebook.com/sharer/sharer.php?u='+encodeURIComponent(href), 'facebook-share-dialog', 'width=626,height=436'); return false;"><img id="face_btn" src="img/modal/facebook_btn.png" alt="facebook" style="width: 28%" /></a> &nbsp; &nbsp; &nbsp; 

						<script type="text/javascript" src="/platform.twitter.com/widgets.js"></script>

						<a href="https://twitter.com/intent/tweet?text=Want%20to%20save%20money%20on%20items%20you're%20currently%20buying?%20Want%20to%20make%20money%20as%20well?%20Check%20this%20out&url=http%3A%2F%2Fcashbackkaboom.com/3735"><img id="twit_btn" src="img/modal/twitter_btn.png" style="width: 28%" alt="Tweet" /></a>
     				</p> 
				</form>
			</div>
			<button class="md-close"></button>
		</div>
	</div>
</div>

<div class="md-overlay"></div><!-- the overlay element -->


<!-- Register Modal -->
<!-- End Register Modal -->



<div class="clear"></div>

<?php include 'user/footer.php';?>
 