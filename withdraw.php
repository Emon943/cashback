...
<h1 class="pagetitle">Request Payment</h1>
<p id="withdraw_sub">
    Enter your desired withdrawal amount below. You must request a minumum of $50<br/>
    from your available balance.<br />
</p>
    ...
    <div id="network_page">
        <div class="network_header">
            <form action="" id="withdraw_money" method="POST">
                <p class="withdraw_directive">Withdraw this amount</p>
                <input type="text" size="25" value="" placeholder="$ Enter Amount" class="textbox" style="width:90%;" name="amount" />
                <br /><br />
                                <input type="radio" name="method" value="check" id="method-check" checked>
                <label for="method-check">I would like to receive my payment as a check</label>
                <br />
                                <input type="radio" name="method" value="paypal" id="method-paypal" >
                <label for="method-paypal">I would like to funds transferred to my account via PayPal</label>
                <br />
                                <br />
                <div class="paypal-email-container" style="display:none;">
                    <p>Paypal Email Address</p>
                    <input type="text" value="motiur943@gmail.com" class="textbox" name="paypal_email" id="paypal-email" />
                </div>
                <button type="submit" class="btn payment-type">Request Payment</a>
            </form>
            <img id="smaller_fat_stack" src="images/smaller_fat_stacks.png" />
        </div>
    </div>


<div id="terms" style="display:none;">
    You may request payout only if your current account balance is greater than or equal to $50.00
</div>

<script type="text/javascript">
jQuery(document).ready(function($){
    var min_payout_amount = 50;
    var user_balance = 0.00;
    $('.btn.payment-type').click(function(){
        if(user_balance < min_payout_amount){
            $('#terms').dialog({modal:true});   
        }
    });
    $("body").on("click", "input[name=method]", function(){
        if($(this).attr("id") == "method-paypal"){
            $(".paypal-email-container").show();
        } else  {
            $(".paypal-email-container").hide();
        }
    });
    $("body").on("submit", "#withdraw_money", function(){
        if($("input[name=method]:checked").val() == "paypal" && $("#paypal-email").val() == ""){
            alert("Please provide your paypal e-mail address if you would like to receive your payment with this method.");
            return false;
        }
    });
});
